...
- URL: `<HTTPS API 接入点>/api/connect/1/?project=<project>&product=<product>&sn=<sn>`
  URL样例：`https://api.eztcloud.com/api/connect/1/?project=PR_q9p7maKXgY&product=PK_kkfGM&sn=1000000066`
...
- URL: `<HTTPS API 接入点>/api/connect/2/?product=<product>&sn=<sn>`
  URL样例：`https://api.eztcloud.com/api/connect/2/?product=PK_kkfGM&sn=1000000066`
  - `HTTPS API 接入点`：可在 `控制台` > `项目设置` > `应用层API` 中获取
- 方法: `POST` 或 `GET`
- 需要认证或鉴权: 否
- 熔断频率: 基于IP，1000次/每小时
